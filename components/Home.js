...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-8gug.vercel.app/movie')
    ...
    //  const overview = data.overview.substr(0,250)+ "..."; 
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.original_title} overview={data.overview.substr(0,250)+ "..."} poster= {`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
